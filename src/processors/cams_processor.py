"""
CAMS processor — full ETL pipeline per attachment.
...
(DBF columns are already ALLCAPS matching BQ external table schema)
    │
    ▼
GcsUploadService.upload_csv()  →  gs://bucket/r2_cams/year=.../...csv
    │
    ▼
BigQueryMergeService.run_merge(CAMS_MERGE_SQL)
"""
...
from src.processors.schema.cams_schema import CAMS_MERGE_SQL

logger = logging.getLogger(__name__)
_TAG = "CAMS"


class CamsProcessor(FileProcessorStrategy):
    """
    Async strategy for CAMS (Computer Age Management Services) attachments.

    Expects:
      - Direct .csv / .dbf attachment, or
    ...
    async def extract(
        ...
        if attachment_filename.lower().endswith(".zip"):
            ...
            if file_map is None:
                return []   # alert already fired inside _extract_zip_files
            if not file_map:
    ...
    async def _process_file(
        ...
        # DBF columns are ALLCAPS (AMC_CODE, FOLIO_NO …) matching BQ external table schema.
        ...
        # Run BigQuery MERGE (R2 only — other report types have no MERGE SQL defined)
        if self._bq_service is not None and report_type == "R2":
            try:
                await self._bq_service.run_merge(CAMS_MERGE_SQL)
            except Exception as exc:
